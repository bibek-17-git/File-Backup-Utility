...
public:
    ...
    void setupCallbacks() {
        ...
        m_view->onRemoveSelected([this]() {
            handleRemoveSelected();
        });
        
        m_view->onClearAll([this]() {
            handleClearAll();
        });
        
        m_view->onStartBackup([this]() {
            handleStartBackup();
        });
    ...
    void handleFileSelection(const std::vector<std::string>& paths, bool recursive) {
        ...
        m_view->updateStatus(std::to_string(m_model->getItemCount()) + " items ready", 0.0);
    }
    ...
    void handleOpenSettings() {
        // SettingsController handles this
    }
    
    void handleViewLog() {
        // Show log dialog
        m_view->showNotification("Log", "Backup logs are saved in the backup directory");
    }
    
    // IBackupObserver implementation
